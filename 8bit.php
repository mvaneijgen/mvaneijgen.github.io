<!doctype html>  
<html lang="en">
    
    <head>
        <meta charset="utf-8">
        
        <title>Jong creatief enzo</title>

        <meta name="description" content="Wij laten zien dat je geen doorgewinterde ondernemer hoeft te zijn om een bedrijf op te richten. Je moet weten waar je voor staat en je hart in je werk stoppen.">
        <meta name="author" content="M van Eijgen">
        
        <link href='http://fonts.googleapis.com/css?family=Open+Sans:400,600' rel='stylesheet' type='text/css'>
        <script>
        if (!("ontouchstart" in document.documentElement)) {
            document.documentElement.className += " no-touch";
            }
        </script>
        <script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></script>
        <link rel="stylesheet" href="assets/css/style.css">
        <!-- Begin meny -->
        <meta name="viewport" content="width=800, user-scalable=no">
        <script src="assets/js/meny.js" type="text/javascript"></script>
        <script src="assets/js/script.js" type="text/javascript"></script>
        <!-- End meny -->
                    
    </head>
    
    <body>
        <? include("meny.php"); ?>
        <div class="contents">
            <object id="flash_504471" type="application/x-shockwave-flash" style="width:100%; height:100%;" data="http://fontstruct.com/widget.swf">
                <param name="movie" value="http://fontstruct.com/widget.swf" />
                <param name="wmode" value="opaque" />
                <param name="bgcolor" value="#000000" />
                <param name="allowFullScreen" value="true" />
                <param name="flashvars" value="d=dD0wJmFtcDtwdD0xJmFtcDtpbT0wJmFtcDtsPTEmYW1wO3A9MCZhbXA7cHU9MCZhbXA7Y2RuPWh0dHA6Ly9zdGF0aWMuZm9udHN0cnVjdC5jb20mYW1wO2Y9NTA0NDcx" />
            </object>

        </div>
   </body>
</html>